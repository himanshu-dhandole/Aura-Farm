...
/**
 * @title VaultViews
 * @notice Separate contract for view functions
 */
contract VaultViews is VaultStorage {
    ...
    function _getTierAssets(uint8 tier) internal view returns (uint256) {
        uint256 total = 0;
    ...
    function getRiskTierInfo(uint8 riskTier) external view returns (string memory name, uint256 totalAllocated, uint256 strategyCount) {
        require(riskTier < 3, "Invalid tier");
        return (riskTiers[riskTier].name, _getTierAssets(riskTier), riskTiers[riskTier].strategies.length);
    }
...
/**
 * @title AuraVault
 * @notice Main vault contract - significantly reduced
 */
contract AuraVault is ERC20, VaultViews, Ownable, ReentrancyGuard {
    ...
    function addStrategy(uint8 riskTier, address strategy, uint8 allocationPct) external onlyOwner {
        ...
        riskTiers[riskTier].strategies.push(StrategyAllocation({strategy: strategy, allocationPct: allocationPct, active: true}));
    ...
    function updateTierAllocations(uint8 riskTier, uint256[] calldata indices, uint8[] calldata allocations) external onlyOwner {
        ...
        for (uint256 i = 0; i < indices.length; i++) {
            require(indices[i] < riskTiers[riskTier].strategies.length && riskTiers[riskTier].strategies[indices[i]].active && allocations[i] > 0, "Invalid strategy");
    ...
    function _allocateToTier(uint8 tier, uint256 amount) internal {
        if (amount == 0) return;
        for (uint256 i = 0; i < riskTiers[tier].strategies.length; i++) {
            if (riskTiers[tier].strategies[i].active) {
                uint256 strategyAmount = (amount * riskTiers[tier].strategies[i].allocationPct) / 100;
                if (strategyAmount > 0) {
                    depositToken.approve(riskTiers[tier].strategies[i].strategy, strategyAmount);
                    IStrategy(riskTiers[tier].strategies[i].strategy).deposit(strategyAmount, address(this));
                }
            }
        }
    }

    function _withdrawFromTier(uint8 tier, uint256 amount) internal returns (uint256) {
        ...
        for (uint256 i = 0; i < riskTiers[tier].strategies.length; i++) {
            if (riskTiers[tier].strategies[i].active) {
                uint256 strategyAmount = (amount * riskTiers[tier].strategies[i].allocationPct) / 100;
                if (strategyAmount > 0) {
                    totalWithdrawn += IStrategy(riskTiers[tier].strategies[i].strategy).withdraw(strategyAmount, address(this), address(this));
                }
            }
        }
    ...
    function deposit(uint256 assets) external nonReentrant returns (uint256 shares) {
        ...
        require(riskNFT.hasProfile(msg.sender), "No risk profile");
        ...
        if (lowAlloc > 0) _allocateToTier(0, lowAlloc);
        if (medAlloc > 0) _allocateToTier(1, medAlloc);
        if (highAlloc > 0) _allocateToTier(2, highAlloc);
    ...
    function withdraw(uint256 shares) external nonReentrant returns (uint256 assets) {
        ...
        uint256 tier0Assets = _getTierAssets(0);
        uint256 tier1Assets = _getTierAssets(1);
        uint256 tier2Assets = _getTierAssets(2);
        uint256 totalTierAssets = tier0Assets + tier1Assets + tier2Assets;

        uint256 received = 0;
        
        if (totalTierAssets > 0) {
            if (tier0Assets > 0) received += _withdrawFromTier(0, (assetsToReturn * tier0Assets) / totalTierAssets);
            if (tier1Assets > 0) received += _withdrawFromTier(1, (assetsToReturn * tier1Assets) / totalTierAssets);
            if (tier2Assets > 0) received += _withdrawFromTier(2, (assetsToReturn * tier2Assets) / totalTierAssets);
        }
        ...
        if (dep.totalDeposited > 0) {
            uint256 ratio = (shares * 1e18) / (supplyNow + shares);
